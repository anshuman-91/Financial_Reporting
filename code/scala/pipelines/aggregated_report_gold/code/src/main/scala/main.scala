...
object Main {
  ...
  def apply(spark: SparkSession): Unit = {
    ...
    val df_SubGraph_1 = SubGraph_1.apply(spark, df_Products)
    val df_Join_products =
      Join_products(spark, df_join_transactions, df_SubGraph_1)
    val df_People       = People(spark)
    val df_SubGraph_1_1 = SubGraph_1_1.apply(spark, df_People)
    val df_Join_people  = Join_people(spark,        df_Join_products, df_SubGraph_1_1)
    Target_1(spark, df_Join_people)
  }
